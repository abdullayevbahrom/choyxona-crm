...
{
    public function __construct(private readonly ActivityLogQueryService $queryService)
    {
    }
    ...
    public function exportCsv(Request $request): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($validated) {
            ...
            $this->queryService->build($validated)
                ->with('user')
    ...
    public function downloadExport(ActivityLogExport $export): BinaryFileResponse|RedirectResponse
    {
        if ($export->user_id !== auth()->id()) {
            abort(403);
        }

        if ($export->status !== ActivityLogExport::STATUS_READY || ! $export->file_path) {
